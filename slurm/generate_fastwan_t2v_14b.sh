#!/bin/bash
#SBATCH --job-name=fastwan-t2v-14b
#SBATCH --partition=gpu
#SBATCH --gres=gpu:h100-96:1
#SBATCH --cpus-per-task=8
#SBATCH --mem=128G
#SBATCH --time=02:00:00
#SBATCH --output=fastwan-t2v-14b-%j.out
...
OUTPUT_DIR=${OUTPUT_DIR:-"$SCRATCH/sglang/outputs/fastwan_t2v_14b"}
mkdir -p "$HF_HOME" "$TMPDIR" "$XDG_CACHE_HOME" "$TORCHINDUCTOR_CACHE_DIR" "$TRITON_CACHE_DIR" "$OUTPUT_DIR"

RUN_PREFIX=${RUN_PREFIX:-"fastwan_t2v_14b_${SLURM_JOB_ID:-manual}"}
...
echo "FastWan T2V 14B DMD generation completed."
echo "Summary CSV: $SUMMARY_CSV"
echo "Outputs: $OUTPUT_DIR/${RUN_PREFIX}_run_*.mp4"
echo "Perf JSON files: $OUTPUT_DIR/${RUN_PREFIX}_perf_run_*.json"
